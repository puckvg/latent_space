...
def read_properties_b3lyp(filename, values={}):
    ...
    # im frequencies 
    start_idx = get_rev_index(lines, "constant matrix")
    if get_rev_index(lines, "imaginary frequencies")!=None: 
        end_idx = get_rev_index(lines, "imaginary frequencies (negative Signs)")
    elif get_rev_index(lines, "DSYEVD-2 returned Info")!=None:
        end_idx = get_rev_index(lines, "DSYEVD-2 returned Info")
    else:
